...
build: check-go check-git
	...
	go build -o polygon-edge -ldflags="\
    	-X 'github.com/Pawswap/posa/versioning.Version=$(VERSION)' \
		-X 'github.com/Pawswap/posa/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/Pawswap/posa/versioning.Branch=$(BRANCH)'\
		-X 'github.com/Pawswap/posa/versioning.BuildTime=$(TIME)'" \
	main.go
